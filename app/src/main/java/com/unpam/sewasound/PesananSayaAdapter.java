...
public class PesananSayaAdapter  extends RecyclerView.Adapter<PesananSayaViewHolder> {
    private ArrayList<TransaksiItem> mdataList;
    private Context context;
    AlertDialog alert;
    public PesananSayaAdapter(Context context, @NonNull ArrayList<TransaksiItem> dataList) {
        mdataList = dataList;
    ...
    public PesananSayaViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        ...
        return new PesananSayaViewHolder(view);
    }


    @UiThread
    @Override
    public void onBindViewHolder(final PesananSayaViewHolder parentHolder, final int position) {
        if(position != RecyclerView.NO_POSITION){
            parentHolder.onBind(mdataList, position);
            parentHolder.btnCancel.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    Trx(mdataList.get(position).getIdPenyewaan(), context, position, StatusConstant.CANCEL, "Penyewaan dibatalkan");
                }
            });
            parentHolder.btnFinish.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    Trx(mdataList.get(position).getIdPenyewaan(), context, position, StatusConstant.FINISH, "Penyewaan Selesai");
                }
            });
    ...
    public void Trx (final String id, final Context context, int position, final int status, String txtStatus){
